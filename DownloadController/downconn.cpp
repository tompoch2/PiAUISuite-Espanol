...
int Downloader::Search(string download, string *link, bool verify) {
    ...
    if (regex_search(curlbuf.c_str(), m, rexp)) {
        ...
        *link = t;
        ...
        cout << "No se pudo encontrar el torrent. Inténtalo de nuevo.\n";
        return -1;
    }
...
int Downloader::DownloadTorrent(string torrent) {
    ...
    if (!pf) {
        cout << "No se pudo abrir el canal de comandos. Dying\n";
        return -1;
    }
    ...
    if (pclose(pf) != 0) {
        cout << "No se pudo cerrar el canal de comandos. Dying\n";
        return -1;
    }
