...
int main(int argc, char *argv[])
{
    ...
    if(gv.Init()) {cout << "GoogleVoice () no se pudo inicializar. Culpar a curl. Dying.\n"; return -1;}
    ...
    if(!results.empty()) {
        ...
        if(!pf) {cout << "No se pudo abrir la canalización de comandos. Dying\n"; return -1;}
        ...
        if (pclose(pf) != 0) {cout << "No se pudo cerrar el canal de comandos. Dying\n"; return -1;}
        replace_all(message, "\t", " ");
        //Send return SMS
	    ...
	    if(gv.debug&1 && !r) printf("Envío de SMS realizado con éxito.\n");
    }    
	return r;
}
