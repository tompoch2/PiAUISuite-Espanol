...
int GoogleVoice::SendSMS(string number, string msg)
{
	...
	if(cr!=CURLE_OK) {cout << "curl() error al enviar SMS: " << errorbuf << endl; return -3;}
	...
	else {cout << "Algo salió mal. Habilitando depuración.\n"; return -1;}
...
int GoogleVoice::MarkAsRead(string msg_id)
{
	...
	curl_easy_setopt(hcurl, CURLOPT_POSTFIELDS, data.c_str());	// TODO: check this and make sure it doesnt hold on to passed data ptr.
	curlbuf.clear(); cr=curl_easy_perform(hcurl);
	if(cr!=CURLE_OK) {cout << "curl() error al marcar SMS como leído: " << errorbuf << endl; return -3;}
    return 0;
}

int GoogleVoice::GetContactInfo() {
	if(!hcurl) {cout << "hcurl es NULL. ¿Se le olvidó llamar a Init ()?\n"; return -1;}
...
int GoogleVoice::DeleteSMS(string msg_id)
{
	...
	curl_easy_setopt(hcurl, CURLOPT_POSTFIELDS, data.c_str());	// TODO: check this and make sure it doesnt hold on to passed data ptr.
	curlbuf.clear(); cr=curl_easy_perform(hcurl);
	if(cr!=CURLE_OK) {cout << "curl() error al eliminar SMS: " << errorbuf << endl; return -3;}
    return 0;
}
...
int GoogleVoice::CallNumber(string to, string from) {
	if(!hcurl) {cout << "hcurl is NULL. ¿Se le olvidó llamar a Init()?\n"; return -1;}
	if(Login()) return -1;
...
int GoogleVoice::CheckSMS(string &results, string number, string keyword, bool delete_sms)
{
	...
	curlbuf.clear(); cr=curl_easy_perform(hcurl);
	if(cr!=CURLE_OK) {cout << "curl() error al obtener SMS: " << errorbuf << endl; return -3;}
...
int GoogleVoice::Login()
{
	...
	{ 
		...
		galx=m[2];
		if(debug&1) cout << "Tiene un token de sesión GALX: " << galx << endl;

	} else {cout << "No se pudo encontrar el token GALX.\n"; return -2;}
	...
	{ 
		...
		if(debug&1) cout << "Tiene un token de sesión rnr_se: " << rnr_se << endl;
	}
	else
	{
		cout << "Error al encontrar el token rnr_se. (Lo más probable es un error/mal escrito e-mail/passwd)\n";
		loggedin=0;
		return -2;
	}
    FILE *fp;
    fp = fopen(DTFILE,"w");
    if(fp == NULL) {
        printf("Tiempo de escritura de ERROR para cookies\n");
        exit(-1);
    }
    time_t now;
    time(&now);
