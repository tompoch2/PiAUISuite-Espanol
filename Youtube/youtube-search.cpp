...
int main(int argc, char *argv[]) {
    ...
    if (tube.Init()) {
        cout << "Curl No se pudo inicializar. Dying.\n";
        return -1;
    }
    string link;
...
void PrintUsage(void) {
    printf("Simplemente escriba lo que desea que busque como en el sitio web.\nEj: youtube-search alguna cosa\n");
...
int Youtube::Search(string query, string *link, bool verify) {
    ...
    if (!hcurl) {
        cout << "hcurl is NULL.  ¿Se le olvidó llamar a Init()?\n";
        return -1;
    }
    ...
    if (regex_search(curlbuf.c_str(), m, rexp)) {
        ...
        *link = t;
        ...
        cout << "No se pudo encontrar el video. Inténtalo de nuevo.\n";
        return -1;
    }
...
int Youtube::PlayVideo(string video, bool verify) {
    ...
    if (!pf) {
        cout << "No se pudo abrir el canal de comando. Dying\n";
        return -1;
    }
    ...
    if (pclose(pf) != 0) {
        cout << "No se pudo cerrar el canal de comando. Dying\n";
        return -1;
    }
